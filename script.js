// ═══════════════════════════════════════════════════════════════
// ИНИЦИАЛИЗАЦИЯ ГАЛЕРЕИ И ЛАЙТБОКСА
// ═══════════════════════════════════════════════════════════════
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Новая кнопка скачивания избранного
    const downloadFavBtn = document.getElementById('download-fav-btn');
    ...
    function handleStorageError() {
        if (confirm('Память браузера переполнена (лимит 5МБ). Это происходит из-за слишком большого количества фото.\n\nХотите очистить всю галерею, чтобы освободить место?')) {
    ...
    function updateFavBadge() {
        ...
        if (count > 0) {
            ...
            if (downloadFavBtn) downloadFavBtn.style.display = 'flex';
        } else {
            favCountBadge.style.display = 'none';
            if (downloadFavBtn) downloadFavBtn.style.display = 'none';
        }
    }
    ...
    function toggleFavorite(card) {
        ...
        // Если лайтбокс открыт и это та же карточка, обновляем кнопку сердца в лайтбоксе
        if (lbOverlay.classList.contains('lb-active') && lbCards[lbIndex] === card) {
            updateLbHeartState();
        }
    }
    ...
    function lbClose_() {
        ...
        setTimeout(() => {
            if (!lbOverlay.classList.contains('lb-active')) {
                lbImg.src = '';
            }
        }, 300);
    ...
    function updateLbHeartState() {
        const card = lbCards[lbIndex];
        if (!card) return;
        const isFav = favorites.has(card.dataset.id);
        if (isFav) {
            lbHeart.classList.add('lb-favorited');
            lbHeart.querySelector('i').className = 'fa-solid fa-heart';
        } else {
            lbHeart.classList.remove('lb-favorited');
            lbHeart.querySelector('i').className = 'fa-regular fa-heart';
        }
    }
    ...
    function toggleZoom() {
        ...
        if (lbZoomed) {
            ...
            lbZoomBtn.querySelector('i').className = 'fa-solid fa-magnifying-glass';
            lbImgWrap.style.overflow = 'hidden';
            lbImgWrap.scrollTo(0, 0);
        }
    }

    // Слушатели событий лайтбокса
    ...
    lbHeart.addEventListener('click', (e) => {
        e.stopPropagation();
        const card = lbCards[lbIndex];
    ...
    lbZoomBtn.addEventListener('click', (e) => { e.stopPropagation(); toggleZoom(); });
    ...
    lbOverlay.addEventListener('touchend', (e) => {
        ...
        if (Math.abs(dx) > 50 && !lbZoomed) lbGo(dx < 0 ? 1 : -1);
    }, { passive: true });

    // ═══════════════════════════════════════════════════════════════════════════
    //  ПАНЕЛЬ ИЗБРАННОГО (ОТРИСОВКА)
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    function renderFavoritesPanel() {
        ...
        favCards.forEach(card => {
            ...
            clone.querySelector('.fav-img-click').addEventListener('click', () => {
                const index = getLbCards().indexOf(card);
    ...
    /** Скачивание только избранных фото в ZIP */
    if (downloadFavBtn) {
        downloadFavBtn.addEventListener('click', async (e) => {
            e.stopPropagation();
            const allCards = getLbCards();
            const favImages = allCards
                .filter(c => favorites.has(c.dataset.id))
                .map(c => c.querySelector('img'));

            if (favImages.length === 0) return;

            const originalText = downloadFavBtn.innerHTML;
            downloadFavBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Сжатие...';
            downloadFavBtn.disabled = true;

            const zip = new JSZip();
            try {
                await Promise.all(favImages.map(async (img, index) => {
                    try {
                        const response = await fetch(img.src);
                        const blob = await response.blob();
                        zip.file(`favorite_photo_${index + 1}.jpg`, blob);
                ...
                const a = document.createElement('a');
                a.href = zipUrl;
                a.download = 'My_Favorites_GrandPalace.zip';
                document.body.appendChild(a);
                a.click();
                document.body.removeChild(a);
                ...
                downloadFavBtn.innerHTML = originalText;
                downloadFavBtn.disabled = false;
            }
        });
    }
    ...
    function attachCardEvents(card) {
        // Клик по карточке → открыть лайтбокс (если не по иконкам)
        card.addEventListener('click', (e) => {
            if (e.target.closest('.overlay-icons') || e.target.tagName.toLowerCase() === 'i') return;
        ...
        // Отдельный клик на иконку лупы
        const zoomIcon = card.querySelector('.fa-magnifying-glass');
        if (zoomIcon) {
    ...
    /** Сжатие изображения через Canvas перед сохранением в localStorage */
        ...
        Array.from(files).forEach(file => {
            if (file.type.startsWith('image/')) {
                ...
                reader.onload = async (event) => {
                    ...
                    // Сжимаем перед добавлением в сетку и сохранением
                    const compressedBase64 = await compressImage(originalBase64);
                    addPhotoToGrid(compressedBase64, file.name, true);
                };
    ...
    downloadAllBtn.addEventListener('click', async () => {
        const zip    = new JSZip();
        const images = Array.from(document.querySelectorAll('.photo-grid .photo-card img'));

        if (images.length === 0) { alert('Нет фотографий для скачивания'); return; }
        ...
        try {
            ...
            if (Object.keys(zip.files).length === 0) {
                alert('Не удалось скачать демо-фотографии (CORS). Ваши фото скачиваются без проблем.');
                return;
            }
    ...
    document.querySelectorAll('.photo-grid .photo-card').forEach(attachCardEvents);
    updateFavBadge();
});
